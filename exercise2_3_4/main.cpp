...
int main() {
	...
	//cp.X(0.3);//this def gives me an error. Red squiggle under X with message:
	/*no instance of overloaded function "Point::X" matches the argument list
	list and the object etc etc.*/
	cout << "X coordinate is: " << cp.X() << endl;
	cout << "Y coordinate is: " << cp.Y() << endl;
	cout<< "Distance from origin is: " << cp.Distance() << endl;
	cp.ToString();
