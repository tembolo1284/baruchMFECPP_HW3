...
void print(const Point& pnt) {
	cout << "Your point is: (" << pnt.getX() << "," << pnt.getY() << ")" << endl;
}
...
int main() {
	...
	pnt1.setX(x_pt); //calls setx from class and binds x_pt's value to x for pnt1
	pnt1.setY(y_pt); //same as above but for the y coordinate
	...
	cout << "Distance from your pnt to the origin is: " << pnt3.DistanceOrigin() << endl; //should be (2^2 +1^2)^0.5 = sqrt of 5
